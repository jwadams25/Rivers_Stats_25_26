...
::: {style="background-color: white; padding: 15px; border: 5px solid #FF585D; border-radius: 5px;"}
Any text highlighted in this red border will require you to either **edit and run a code chunk or just run a code chunk**.
:::

::: {style="background-color: white; padding: 15px; border: 5px solid #006CB3; border-radius: 5px;"}
...
No, you will not need to change the variable on the x-axis as we are still going to explore the distribution of loan amounts.
:::
...
No. A density curve is continuous and therefore can only be used for numeric variables taht are continuous.
:::
...
loan50_small |>
  ...
  scale_x_continuous(limits = c(0, 450000), breaks = seq(0, 450000, 50000)) +
  theme_minimal()
...
loan50_small |>
  ...
  scale_x_continuous(limits = c(0, 450000), breaks = seq(0, 450000, 25000)) +
  theme_minimal() +
